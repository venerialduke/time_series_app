...
from exploration.univariate.arma import (
    ...
    get_all_presets, apply_shock_to_arma
)
...
# Shock configuration
st.sidebar.subheader("Impulse Response (Shock)")
add_shock = st.sidebar.checkbox(
    "Add Shock to Series",
    value=False,
    help="Add a one-time shock to see how the ARMA process responds"
)
...
if add_shock:
    shock_time = st.sidebar.slider(
        "Shock Time (observation #)",
        min_value=10,
        max_value=n_samples - 10,
        value=min(100, n_samples // 2),
        help="Time point when shock occurs (0-based index)"
    )
    ...
    shock_magnitude = st.sidebar.slider(
        ...
        value=3.0,
        step=0.5,
        help="Size of shock in standard deviations"
    )

    st.sidebar.info(f"💡 Shock applied at t={shock_time} with magnitude {shock_magnitude:.1f}σ")
...
if ar_order > 0:
    ...
    if is_stationary:
        ...
        st.warning("⚠ AR process is NOT stationary. Series will still be generated to show behavior.")
        st.write(f"Root magnitudes: {[f'{abs(r):.3f}' for r in ar_roots]} (need > 1.0 for stationarity)")
...
if ma_order > 0:
    ...
    if is_invertible:
        ...
        st.warning("⚠ MA process is NOT invertible. Series will still be generated to show behavior.")
        st.write(f"Root magnitudes: {[f'{abs(r):.3f}' for r in ma_roots]} (need > 1.0 for invertibility)")

# Simulate ARMA process
try:
    shock_info = None

    if add_shock and shock_time is not None and shock_magnitude is not None:
        # Simulate with shock
        series, shock_info = apply_shock_to_arma(
        ...
        st.info(f"📊 Shock of {shock_magnitude:.1f}σ applied at observation {shock_time}. Observe how the series responds and returns to equilibrium.")
    else:
    ...
    try:
        title_suffix = " (with Shock)" if shock_info else ""
        fig_ts = plot_time_series(series, title=f"ARMA({ar_order}, {ma_order}) Process{title_suffix}", shock_info=shock_info)
        st.plotly_chart(fig_ts, width='stretch')
    except Exception as plot_error:
        st.error(f"Error creating time series plot: {str(plot_error)}")
        st.info("Displaying series without shock marker...")
        fig_ts = plot_time_series(series, title=f"ARMA({ar_order}, {ma_order}) Process", shock_info=None)
        st.plotly_chart(fig_ts, width='stretch')
...
- **Add a shock** to see impulse response behavior:
  - AR processes show persistent effects
  - MA processes show temporary effects
  - Higher AR coefficients = slower decay
""")
