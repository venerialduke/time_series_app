...
try:
    ...
    # Build LaTeX formula string
    latex_parts = []
    if enable_trend:
        if trend_type == 'linear':
            latex_parts.append(f"{trend_intercept:.2f} + {trend_slope:.4f}t")
        elif trend_type == 'quadratic':
            latex_parts.append(f"{trend_intercept:.2f} + {trend_slope:.4f}t + {trend_quad:.6f}t^2")
        elif trend_type == 'exponential':
            latex_parts.append(f"{trend_intercept:.2f} e^{{{trend_slope:.4f}t}}")

    if enable_seasonality:
        if seasonal_type == 'additive':
            latex_parts.append(f"{seasonal_amplitude:.2f}\\sin\\left(\\frac{{2\\pi t}}{{{seasonal_period}}}\\right)")
        else:
            latex_parts.append(f"\\left[1 + {seasonal_amplitude:.2f}\\sin\\left(\\frac{{2\\pi t}}{{{seasonal_period}}}\\right)\\right]")
    ...
    if enable_arma:
        if ar_order > 0 or ma_order > 0:
            ...
            if ar_order > 0:
                for i in range(ar_order):
                    arma_terms.append(f"\\phi_{{{i+1}}}y_{{t-{i+1}}}")
            if ma_order > 0:
                for i in range(ma_order):
                    arma_terms.append(f"\\theta_{{{i+1}}}\\varepsilon_{{t-{i+1}}}")
            latex_parts.append(" + ".join(arma_terms))
        latex_parts.append("\\varepsilon_t")

    if len(latex_parts) == 0:
        latex_formula = "y_t = \\varepsilon_t"
    elif seasonal_type == 'multiplicative' and enable_seasonality:
        # For multiplicative seasonality
        non_seasonal = [p for p in latex_parts if 'sin' not in p]
        seasonal_part = [p for p in latex_parts if 'sin' in p]
        if non_seasonal and seasonal_part:
            latex_formula = f"y_t = \\left({' + '.join(non_seasonal)}\\right) \\times {seasonal_part[0]}"
        else:
            latex_formula = f"y_t = {' + '.join(latex_parts)}"
    else:
        latex_formula = f"y_t = {' + '.join(latex_parts)}"
    ...
    # Show parameters if ARMA
    if enable_arma and (ar_order > 0 or ma_order > 0):
        param_str = ""
        if ar_order > 0:
            ar_vals = ', '.join([f"\\phi_{{{i+1}}}={ar_params[i]:.2f}" for i in range(ar_order)])
            param_str += ar_vals
        if ma_order > 0:
            ma_vals = ', '.join([f"\\theta_{{{i+1}}}={ma_params[i]:.2f}" for i in range(ma_order)])
            if param_str:
                param_str += ", "
            param_str += ma_vals
        st.latex(f"\\text{{Parameters: }} {param_str}")

    if enable_shock and shock_time:
        st.latex(f"\\text{{Shock: }} {shock_magnitude:.1f}\\sigma \\text{{ at }} t={shock_time}")
