...
try:
    ...
    # Build VAR equations in LaTeX - use simple variable names to avoid LaTeX issues
    latex_lines = []
    for i in range(n_vars):
        eq_terms = []
        for lag in range(var_order):
            for j in range(n_vars):
                ...
                if abs(coef) > 0.001:  # Only show non-zero coefficients
                    sign = '+' if coef >= 0 else '-'
                    abs_coef = abs(coef)
                    # Use Y1, Y2, etc. for cleaner LaTeX
                    eq_terms.append(f"{sign} {abs_coef:.3f}Y_{{{j+1},t-{lag+1}}}")

        if eq_terms:
            # Join terms, handling first term sign
            terms_str = ''.join(eq_terms)
            if terms_str.startswith('+'):
                terms_str = terms_str[1:].strip()
            eq_line = f"Y_{{{i+1},t}} &= {terms_str} + \\varepsilon_{{{i+1},t}}"
        else:
            eq_line = f"Y_{{{i+1},t}} &= \\varepsilon_{{{i+1},t}}"
        latex_lines.append(eq_line)

    # Display equations in LaTeX
    latex_system = r'\begin{align*}' + '\n' + r' \\'.join(latex_lines) + '\n' + r'\end{align*}'
    st.latex(latex_system)

    # Show variable mapping
    var_mapping = ', '.join([f"Y_{i+1}={var_names[i]}" for i in range(n_vars)])
    st.caption(f"Variables: {var_mapping}")

    if shock_info:
        st.latex(f"\\text{{Shock: }} {shock_magnitude:.1f}\\sigma \\text{{ to }} Y_{{{shock_var+1}}} \\text{{ at }} t={shock_time}")
