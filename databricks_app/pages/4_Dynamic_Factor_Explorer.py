...
try:
    ...
    # Build equations in LaTeX
    st.latex(r'''
    \begin{align*}
    y_t &= \Lambda f_t + \varepsilon_t \\
    f_{k,t} &= \phi_k f_{k,t-1} + \eta_{k,t}
    \end{align*}
    ''')

    # Display parameters
    phi_str = ', '.join([f"\\phi_{i+1}={factor_ar_params[i]:.2f}" for i in range(n_factors)])
    st.latex(f"\\text{{{n_series} series, {n_factors} factors}}, \\quad {phi_str}")
    st.latex(f"\\sigma^2_f = {factor_var:.2f}, \\quad \\sigma^2_\\varepsilon = {idiosyncratic_var:.2f}")

    if info['shock_info']:
        st.latex(f"\\text{{Shock: }} {shock_magnitude:.1f}\\sigma \\text{{ to Factor {shock_factor+1} at }} t={shock_time}")
